...
let departmentNameRemark = document.getElementById('departmentnameremark');
...
tbodySection.addEventListener('click', (e) => {
  ...
  if (editbtn) {
    ...
    let departmentNameRemarkEl = parent.querySelector(
      '.departmentNameRemark'
    ).textContent;

    departmentName.value = departmentNameEl;
    departmentNameRemark.value = departmentNameRemarkEl;
    ...
    btnSaveCompany.addEventListener('click', (e) => {
      ...
      fetch('http://localhost:3000/departments/' + departmentNameId, {
        ...
        body: JSON.stringify({
          ...
          departmentnameremark: departmentNameRemark.value,
        }),
      }).then((res) => res.json());
      resetDepartment();
      location.reload();
      ...
      departments.map((departmentList, idx) => {
        departmentTable += `
          <tr data-id=${departmentList._id}>
            ...
            <td class="departmentNameRemark">${
              departmentList.departmentnameremark
            }</td>
          </tr>`;
      });
